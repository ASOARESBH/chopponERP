            ...
            if ($action === 'add' || $action === 'edit') {
                ...
                if ($action === 'add') {
                    $stmt = $conn->prepare("
                        ...
                        (estabelecimento_id, descricao, tipo, valor, data_vencimento, codigo_barras, link_pagamento, observacoes) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    ");
...
function pagarConta(conta) {
    document.getElementById('pagarModal').style.display = 'block';
