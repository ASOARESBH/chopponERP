...
require_once '../includes/RoyaltiesLogger.php';

$conn = getDBConnection();

// Inicializar logger
$logger = new RoyaltiesLogger('royalties');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['action'])) {
            ...
            if ($action === 'add') {
                $logger->startOperation('Criar Royalty', ['post_data' => $_POST]);
                
                // Validar permissões
                $estabelecimento_id = isAdminGeral() ? intval($_POST['estabelecimento_id']) : getEstabelecimentoId();
                $logger->debug('Estabelecimento ID identificado', ['estabelecimento_id' => $estabelecimento_id]);
                ...
                $logger->info('Dados do royalty preparados', [
                    'estabelecimento_id' => $estabelecimento_id,
                    'cnpj' => $cnpj,
                    'periodo' => "$periodo_inicial a $periodo_final",
                    'valor_faturamento' => $valor_faturamento_bruto,
                ...
                $logger->success('Royalty criado com sucesso', ['royalty_id' => $royalty_id]);
                ...
                $stmt = $conn->prepare("
                    SELECT name, document as cnpj, email_alerta as email, phone, address
                ...
                $stmt = $conn->prepare("
                    SELECT r.*, e.name as estabelecimento_nome, e.document as cnpj, e.email_alerta as email, e.phone, e.address
                    FROM royalties r
                ...
                // Nota: A tabela estabelecimentos tem apenas o campo 'address' completo
                // Não há campos separados (number, district, city, state, zip_code)
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="cnpj" id="cnpj_estabelecimento" class="form-control" readonly 
                           style="background-color: #e9ecef;" placeholder="Carregando...">
                    <small class="form-text text-muted">CNPJ preenchido automaticamente</small>
                </div>
...
function loadEstabelecimentoData() {
    ...
    console.log('[ROYALTIES] Carregando dados do estabelecimento ID:', estabelecimentoId);
    
    if (!estabelecimentoId) {
        console.log('[ROYALTIES] Nenhum estabelecimento selecionado, limpando campos');
        document.getElementById('cnpj_estabelecimento').value = '';
        document.getElementById('email_cobranca').value = '';
        return;
    }
    
    console.log('[ROYALTIES] Fazendo requisição AJAX para:', `ajax/get_estabelecimento_email.php?id=${estabelecimentoId}`);
    
    fetch(`ajax/get_estabelecimento_email.php?id=${estabelecimentoId}`)
        .then(response => {
            console.log('[ROYALTIES] Resposta recebida, status:', response.status);
            return response.json();
        })
        .then(data => {
            console.log('[ROYALTIES] Dados recebidos:', data);
            if (data.success) {
                if (data.cnpj) {
                    console.log('[ROYALTIES] Preenchendo CNPJ:', data.cnpj);
                    document.getElementById('cnpj_estabelecimento').value = data.cnpj;
                } else {
                    console.warn('[ROYALTIES] CNPJ não retornado pela API');
                }
                if (data.email) {
                    console.log('[ROYALTIES] Preenchendo e-mail:', data.email);
                    document.getElementById('email_cobranca').value = data.email;
                } else {
                    console.warn('[ROYALTIES] E-mail não retornado pela API');
                }
            } else {
                console.error('[ROYALTIES] Erro ao buscar dados:', data.error);
                alert('Erro ao buscar dados do estabelecimento: ' + data.error);
            }
        })
        .catch(error => {
            console.error('[ROYALTIES] Erro na requisição AJAX:', error);
            alert('Erro ao buscar dados do estabelecimento. Verifique o console para mais detalhes.');
        });
}
