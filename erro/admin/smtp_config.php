...
try {
    $stmt = $conn->query("SHOW TABLES LIKE 'smtp_config'");
    ...
    Logger::error("Erro ao verificar tabela smtp_config", ['error' => $e->getMessage()]);
}
    ...
    if ($action === 'salvar') {
        try {
            ...
            if (!in_array($encryption, ['tls', 'ssl', 'none'])) {
                throw new Exception('Tipo de criptografia inválido');
            }
            ...
            if ($config_existe) {
                ...
                // Inserir
                if (empty($password)) {
                    throw new Exception('Senha é obrigatória para nova configuração');
                }
            ...
            Logger::error("Erro ao salvar configuração SMTP", ['error' => $e->getMessage()]);
        }
    } elseif ($action === 'testar') {
        try {
            // Verificar se EmailSender existe
            if (!file_exists('../includes/EmailSender.php')) {
                throw new Exception('Arquivo EmailSender.php não encontrado');
            }
            ...
            if ($resultado['success']) {
                ...
                $tipo_mensagem = 'success';
                Logger::info("E-mail de teste enviado", ['email' => $email_teste]);
            } else {
            ...
            $tipo_mensagem = 'danger';
            Logger::error("Erro ao enviar e-mail de teste", [
                'email' => $email_teste ?? 'N/A',
                'error' => $e->getMessage()
            ]);
        }
    }
...
try {
    ...
    $config = null;
    Logger::error("Erro ao buscar configuração SMTP", ['error' => $e->getMessage()]);
}
?>
...
<div class="container-fluid mt-4">
    ...
    <?php if ($mensagem): ?>
    <div class="alert alert-<?= htmlspecialchars($tipo_mensagem) ?> alert-dismissible fade show">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        ...
                        <div class="d-grid">
                            <button type="submit" class="btn btn-success" <?= !$config ? 'disabled' : '' ?>>
                                <i class="fas fa-envelope"></i> Enviar E-mail Teste
                            </button>
                        </div>
                        <?php if (!$config): ?>
                        <small class="text-muted d-block mt-2">Configure o SMTP primeiro para testar</small>
                        <?php endif; ?>
                    </form>
                </div>
            </div>
