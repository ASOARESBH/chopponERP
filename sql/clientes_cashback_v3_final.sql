...
-- CHOPPONv1 v3.1.2 - VERSÃO FINAL CORRIGIDA
-- =====================================================
-- 
-- CORREÇÕES:
-- ✅ Sem chaves estrangeiras (evita erro #1215)
-- ✅ VIEW simplificada sem RANK() (compatível com MySQL 5.x)
-- ✅ Testado e validado
-- =====================================================
...
CREATE TABLE IF NOT EXISTS clientes (
    ...
    INDEX idx_nome (nome),
    INDEX idx_pontos (pontos_cashback),
    INDEX idx_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Cadastro de clientes do estabelecimento';
...
CREATE TABLE IF NOT EXISTS clientes_consumo (
    ...
    INDEX idx_bebida_nome (bebida_nome),
    INDEX idx_valor (valor_total)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Histórico de consumo dos clientes';
...
COMMENT='Regras de cashback configuráveis';
...
COMMENT='Histórico de movimentações de cashback';
...
COMMENT='Configurações gerais de cashback por estabelecimento';
...
SELECT 
    ...
    'Cashback Padrão 5%' as nome,
    'Ganhe 5% de volta em pontos em todas as compras' as descricao,
    'percentual' as tipo_regra,
    5.00 as valor_regra,
    1 as ativo,
    0 as prioridade
WHERE NOT EXISTS (SELECT 1 FROM cashback_regras LIMIT 1);
...
SELECT 
    1 as estabelecimento_id,
    1 as ativo,
    1 as permite_resgate,
    10.00 as valor_minimo_resgate
WHERE NOT EXISTS (SELECT 1 FROM cashback_config LIMIT 1);

-- =====================================================
-- Views úteis (VERSÃO SIMPLIFICADA)
-- =====================================================

-- Remover views se já existirem
DROP VIEW IF EXISTS vw_clientes_stats;
DROP VIEW IF EXISTS vw_clientes_ranking;

-- View de Clientes com Estatísticas
CREATE VIEW vw_clientes_stats AS
SELECT 
    c.*,
...
-- View de Ranking de Clientes (VERSÃO SIMPLIFICADA SEM RANK())
-- Esta view retorna os clientes ordenados, mas sem a função RANK()
-- O ranking pode ser calculado pela aplicação PHP se necessário
CREATE VIEW vw_clientes_ranking AS
SELECT 
    c.id,
...
LEFT JOIN clientes_consumo cc ON c.id = cc.cliente_id
WHERE c.status = 1
GROUP BY c.id, c.estabelecimento_id, c.nome, c.email, c.pontos_cashback, c.total_consumido
...
DROP TRIGGER IF EXISTS trg_clientes_consumo_after_insert;
DROP TRIGGER IF EXISTS trg_cashback_historico_after_insert;
...
-- Trigger para atualizar saldo de cashback
DELIMITER $$
...
UNION ALL
SELECT 
    'clientes_consumo' as tabela,
    COUNT(*) as registros
FROM clientes_consumo
UNION ALL
SELECT 
    'cashback_regras' as tabela,
    COUNT(*) as registros
FROM cashback_regras
UNION ALL
SELECT 
    'cashback_historico' as tabela,
    COUNT(*) as registros
FROM cashback_historico
UNION ALL
SELECT 
    'cashback_config' as tabela,
    COUNT(*) as registros
FROM cashback_config;

-- =====================================================
-- Fim do script - INSTALAÇÃO COMPLETA!
-- =====================================================

-- OBSERVAÇÕES IMPORTANTES:
-- 
-- 1. VIEW vw_clientes_ranking foi SIMPLIFICADA
--    - Removida a função RANK() que causava erro
--    - Os clientes são ordenados por total_consumido DESC
--    - Se precisar do ranking numérico, use no PHP:
--    
--    $ranking = 1;
--    foreach ($clientes as $cliente) {
--        echo $ranking . '. ' . $cliente['nome'];
--        $ranking++;
--    }
--
-- 2. Todas as tabelas usam InnoDB
-- 3. Charset: utf8mb4 (suporta emojis)
-- 4. Sem chaves estrangeiras (evita erros)
-- 5. Triggers funcionam automaticamente
--
-- =====================================================
-- PRONTO PARA USO! Acesse admin/clientes.php
-- =====================================================
